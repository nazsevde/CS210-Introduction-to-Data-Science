...
def extract_content_from_page(file_path):
    ...
    date_element = soup.find('div', {'class': 'elementor-widget-theme-post-content'}).find_all('p')[-1]
    date_str = date_element.find('strong').text.strip()
    date_obj = datetime.strptime(date_str, "Source: LENA %d/%m/%Y")
    parsed_data['date'] = date_obj.strftime("%d/%m/%Y")

    # Extracting title
    parsed_data['title'] = soup.find('head').find('title').text

    # Extracting content from the page
    parsed_data['content'] = soup.find('div', {'class': 'elementor-widget-theme-post-content'}).text
